...
summary(m2.math) # lowest AIC = 1,127, inclusive of sex + age + famsize + Medu + Mjob + studytime + failures + schoolsup + famsup + romantic + freetime + goout + absences

m.portu.full <- lm(G3 ~ . , data = portu_grade)
m2.portu <- step(m.portu.full)
summary(m2.portu) # lowest AIC = 1,287, inclusive of school + sex + age + Medu + guardian + studytime + failures + schoolsup + higher + romantic + Dalc + health + absences

m3.math <- step(m.math.full, direction = 'both') # lowest AIC = 1,127, inclusive of sex + age + famsize + Medu + Mjob + studytime + failures + schoolsup + famsup + romantic + freetime + goout + absences
summary(m3.math)

m3.portu<- step(m.portu.full, direction = 'both') # lowest AIC = 1,287, inclusive of school + sex + age + Medu + guardian + studytime + failures + schoolsup + higher + romantic + Dalc + health + absences
summary(m3.portu)
...
levels(math_grade$address) # factor values
levels(math_grade$reason)
levels(portu_grade$higher)
cor(math_grade2$address, math_grade2$reason) # numeric values
cor(portu_grade2$higher, portu_grade2$studytime)
