...
par(mfrow=c(1,2)) 
plot(pve, xlab="Principal Component", ylab="Proportion of Variance SExplained", ylim=c(0,1),type='b')
...
cart.bank <- rpart(y ~., data = trainset.bank, method = 'class', control = rpart.control(minsplit = 2, cp = 0))
print(cart.bank)
...
cart.bank.1SE$cptable
print(row.names(cart.bank.1SE$variable.importance))

cart.bank.1SE.df <- data.frame(cart.bank.1SE$variable.importance)
cart.bank.1SE.df <- cbind(Variables = rownames(cart.bank.1SE.df), cart.bank.1SE.df)
rownames(cart.bank.1SE.df) <- 1:nrow(cart.bank.1SE.df)
colnames(cart.bank.1SE.df)[which(names(cart.bank.1SE.df) == "cart.bank.1SE.variable.importance")] <- "Importance_Index"
# duration and outcome is the most important variable.

# Visualize Variable Importance Index
cart_bank_1SE_plot <- ggplot(cart.bank.1SE.df, aes(x=reorder(Variables, Importance_Index), y=Importance_Index, fill=Importance_Index)) + 
  geom_bar(color="blue", stat = 'identity') +
  coord_flip() +
  ggtitle("Variable Importance Index of Pruned Classification Tree methods for modelling Bank subscription") +
  xlab("Variables") + 
  ylab("Importance Index") 
cart_bank_1SE_plot

# Alternatively, simply plot using:
#barplot(cart.bank.1SE.df$`Importance_Index`, names.arg=cart.bank.1SE.df$Variables)
